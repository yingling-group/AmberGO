...
def calculate_oxygen_num_type(NumC, ONumO, PercentO, HERatio):
    ...
    # Adjust functional group proportions
    HydroxylFraction = 0.7  # Increased to 70%
    DihydroxylFraction = 0.2  # Reduced to 20%
    EpoxyFraction = 0.1  # Reduced to 10%

    OxygenNumAndType = {}
    OxygenNumAndType["Surface_Hydroxyl"] = int(round(NumO * HydroxylFraction, 0))
    OxygenNumAndType["Dihydroxyl"] = int(round(NumO * DihydroxylFraction, 0))
    if (OxygenNumAndType["Dihydroxyl"] % 2) != 0:  # Ensure even number of dihydroxyl
        OxygenNumAndType["Dihydroxyl"] -= 1
        OxygenNumAndType["Surface_Hydroxyl"] += 1
    OxygenNumAndType["Epoxy"] = int(round(NumO * EpoxyFraction, 0))
...
def add_all_dihydroxyl(NewO, NewH, OxygenNumAndType, coords, abc, PossibleDefectSites, NNList, o_up, o_down):
    ...
    for dihydroxyl in range(0, half):
        ...
        # Skip if c1index is in PossibleDefectSites
        if c1index in PossibleDefectSites:
            continue

        # Try to place dihydroxyl
        for neighbor in NNList[c1index]:
            if neighbor in twocarbongroupsites and neighbor not in PossibleDefectSites and not placed:
                c2index = neighbor
                o1Coords, h1Coords, o2Coords, h2Coords = add_dihydroxyl(coords[c1index], coords[c2index], abc)
                placed = True

                # Remove sites if successfully placed
                PossibleDefectSites.append(c1index)
                PossibleDefectSites.append(c2index)

                # Remove neighbors from the defect list
                PossibleDefectSites = [site for site in PossibleDefectSites if site not in NNList[c1index]]
                PossibleDefectSites = [site for site in PossibleDefectSites if site not in NNList[c2index]]

                # Append new coordinates
                NewO.extend([o1Coords, o2Coords])
                NewH.extend([h1Coords, h2Coords])
                o_up += 1
                o_down += 1

        # Skip to next site if placement fails
        if not placed:
            continue
...
def add_all_epoxy(NewO, NewH, OxygenNumAndType, coords, abc, PossibleDefectSites, NNList, NNListSW, o_up, o_down):
    twocarbongroupsites = assess_two_carbon_group(coords, PossibleDefectSites, NNList)
    unplacedepoxy = 0  # Counter for unplaced epoxy groups
    ...
    for epoxy in range(0, OxygenNumAndType["Epoxy"]):
        ...
        try:
            ...
            for neighbor in NNList[c1index]:
                if neighbor in twocarbongroupsites and neighbor not in PossibleDefectSites and not placed:
                    c2index = neighbor
                    oCoords = add_epoxy(coords[c1index], coords[c2index], coin_flip(), abc)
                    placed = True

                    # Mark these sites as occupied
                    PossibleDefectSites.append(c1index)
                    PossibleDefectSites.append(c2index)
    ...
    return NewO, NewH, PossibleDefectSites, o_up, o_down, unplacedepoxy
